...
class PropagationResult:
    ...
    poses: List[Pose] = field(default_factory=list) #tutte le pose generate
...
def compute_arc(
    ...
    2. The centre must lie on the perpendicular to t:
    ...
    """
    #distance from robot(S) to target(G)
    dx = x_g - x_s
    dy = y_g - y_s


    #case: start is on the goal
    dist_sq = dx * dx + dy * dy
    if dist_sq < 1e-12:
    ...
    # how much the target is shifted laterally  respect to the direction of the robot.
    d_perp = dx * nx + dy * ny   # positive → goal is to the left


    #case: goal on the headline→ degenerate straight arc
    if abs(d_perp) < COLLINEAR_TOL: #if lateral displacement almost null
        
        # Check that the goal is ahead (positive tangent projection)
        d_tang = dx * math.cos(theta_s) + dy * math.sin(theta_s)
        if d_tang <= 0:
            return None  # goal is behind-->impossible

        arc_len = min(math.sqrt(dist_sq), STEP_SIZE) #lenght of the movment, limited by a maximum step

        return ArcParams(
            center_x=x_s + MAX_RADIUS * nx,
            center_y=y_s + MAX_RADIUS * ny,
            radius=MAX_RADIUS, #curva = retta
            omega=DEFAULT_VELOCITY / MAX_RADIUS, 
            arc_length=arc_len,
            direction=1,
            degenerate=True, #report that this movment is a straight line
        )
    ...
    direction = 1 if R_signed > 0 else -1 #1 left, -1 negative right

    #circle center coordinates
    cx = x_s + R_signed * nx
    cy = y_s + R_signed * ny

    # Arc angle from start to goal (polar angle)
...
def propagate(
    ...
    override_length: Optional[float] = None, #Sovrascrive la lunghezza dell'arco (usato per goal connection
) -> PropagationResult:
    ...
    """
    poses: List[Pose] = [(x, y, theta)] #list to memorize all the positions
    travelled = 0.0 #counter: how long the robot has already traveled
    step_dist = velocity * dt

    # Clamp the arc length to STEP_SIZE unless overridden
    if override_length is not None:
        effective_length = override_length #if i'm trying to connect whit the final goal
    else:
        effective_length = min(arc.arc_length, STEP_SIZE)
    n_steps = min(int(math.ceil(effective_length / step_dist)), max_steps) #total stps needed

    for _ in range(n_steps): #for every steps, update the remaining stps
        remaining = effective_length - travelled
        if remaining <= 0:
            break
        actual_dt = dt if remaining >= step_dist else remaining / velocity #last step: if the remaining is less then a single step 
    ...
    result.reached_goal = (travelled >= effective_length - 1e-6) #check if the robot reached the goal
    return result
